...
class TelegramBot:
    ...
    def format_portfolio_signal(
        ...
        """Produce the enhanced Telegram message for SPOT/GEM portfolio signals.

        Includes WHY THIS TRADE narrative, sector comparison, and key levels.
        ...
        Format::
            ...
            📈 Confidence: 78.2 (A+)
            🏷 Risk: MEDIUM | Sector: DeFi | MCap: N/A
        """
        chan_emojis = {
            "360_SPOT": "📈",
            "360_GEM": "💎",
        ...
        emoji = chan_emojis.get(sig.channel, "📡")
        chan_name = chan_display.get(sig.channel, TelegramBot._CHANNEL_DISPLAY_NAME.get(sig.channel, sig.channel))
        dir_word = sig.direction.value
        separator = "━" * 28
        ...
        if narrative:
            ...
            for narrative_line in narrative.splitlines():
                stripped = narrative_line.strip()
                if stripped:
                    lines.append(TelegramBot._escape_md(stripped))

        # Sector comparison
        if sector_context is not None:
            lines.append("")
            lines.append("📊 *SECTOR COMPARISON:*")
            sector_name = getattr(sector_context, "sector_name", "N/A")
            sector_7d = getattr(sector_context, "sector_7d_pct", 0.0)
            symbol_7d = getattr(sector_context, "symbol_7d_pct", 0.0)
            rel_str = getattr(sector_context, "relative_strength", "in-line")
            lines.append(
                f"• {TelegramBot._escape_md(sector_name)} sector avg: {sector_7d:+.1f}% (7d)"
            )
            rel_note = (
                "catch-up potential" if rel_str == "lagging"
                else "momentum confirmation" if rel_str == "leading"
                else "in-line"
            )
            lines.append(
                f"• {TelegramBot._escape_md(sig.symbol)}: {symbol_7d:+.1f}% "
                f"({TelegramBot._escape_md(rel_str)} → {TelegramBot._escape_md(rel_note)})"
            )
            peers = getattr(sector_context, "peers", [])
            if peers:
                peer_parts = " | ".join(
                    f"{TelegramBot._escape_md(sym)} {pct:+.1f}%"
                    for sym, pct in peers[:4]
                )
                lines.append(f"• Peers: {peer_parts}")
            corr = getattr(sector_context, "correlated_major", None)
            if corr:
                lines.append(
                    f"• Correlated: {TelegramBot._escape_md(corr[0])} {corr[1]:+.1f}% (7d)"
                )

        # Key levels
        lines.append("")
        lines.append("🔬 *KEY LEVELS:*")
        # Breakout confirmation: entry * 1.07 or TP1 (whichever is lower)
        breakout = min(sig.tp1, sig.entry * 1.07) if sig.entry else sig.tp1
        lines.append(f"• `${fmt_price(breakout)}` — breakout confirmation")
        lines.append(f"• `${fmt_price(sig.stop_loss)}` — invalidation (daily close)")
        lines.append(f"• `${fmt_price(sig.tp1)}` — first major resistance")
        ...
        # Risk / Sector / MCap footer
        effective_risk = risk_label or sig.risk_label or "N/A"
        sector_label = getattr(sector_context, "sector_name", "N/A") if sector_context is not None else "N/A"
        mcap = getattr(sector_context, "market_cap_usd", None) if sector_context is not None else None
        if mcap and mcap > 0:
            ...
            mcap_str = "N/A"
        lines.append(
            f"🏷 Risk: *{TelegramBot._escape_md(effective_risk)}* | "
            f"Sector: {TelegramBot._escape_md(sector_label)} | MCap: {TelegramBot._escape_md(mcap_str)}"
        )
